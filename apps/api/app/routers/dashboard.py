...
def dashboard(range: str = "all", _: User = Depends(get_current_admin), db: Session = Depends(get_db)) -> dict[str, object]:
    ...
    by_language = db.execute(select(Conversation.language, func.count(Conversation.id)).where(Conversation.id.in_([c.id for c in conversations])).group_by(Conversation.language)).all() if conversations else []
